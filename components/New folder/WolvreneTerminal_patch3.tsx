    ...
    return (
      ...
      >
        <div
          ...
          className={`h-[2px] cursor-row-resize ${dashed ? "border-t border-dashed" : "border-t"}`}
        ...
        <div
          ...
          className="absolute right-2 -top-3 flex items-center gap-1 cursor-row-resize"
        >
          <span
            className="rounded-md px-2 py-1 text-[10px] font-black text-black shadow-lg"
            style={{ backgroundColor: hit ? "#ef4444" : color }}
          >
            {hit ? "HIT" : label}
          </span>
          ...
          {onDelete && (
            <button
              ...
              className="hidden group-hover:block rounded-md bg-red-600 px-2 py-1 text-[10px] font-bold text-white"
            >
              ×
            </button>
          )}
      ...
      <div className={fullscreen ? "w-full max-w-[1800px] mx-auto" : "w-full max-w-7xl mx-auto"}>
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-gray-500 text-xs tracking-wide">BTC AI Dashboard v15.7 — Position Manager Pro</p>
          </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_360px] gap-4">
          ...
          {!hideUI && (
            <aside className="space-y-4 min-w-0">
              ...
              <div className={`${card} p-5`}>
                <h2 className="text-sm text-gray-400 font-semibold mb-3">Trade Panel</h2>

                <div className="grid grid-cols-3 gap-2 mb-3 text-[11px]">
                  <button className="rounded-lg bg-zinc-800 py-2 font-bold text-white">Isolated</button>
                  <button className="rounded-lg bg-black border border-zinc-800 py-2 text-gray-500">Cross</button>
                  <button className="rounded-lg bg-black border border-zinc-800 py-2 text-yellow-500">{selectedOrder?.leverage || 5}x</button>
                </div>

                <div className="grid grid-cols-2 gap-2 mb-3 text-xs">
                  <button
                    onClick={() => selectedOrder && updateOrder(selectedOrder.id, { orderType: "LIMIT" })}
                    className={`rounded-lg py-2 border ${selectedOrder?.orderType === "LIMIT" ? "bg-zinc-700 border-zinc-600" : "bg-black border-zinc-800 text-gray-500"}`}
                  >Limit</button>
                  <button
                    onClick={() => selectedOrder && updateOrder(selectedOrder.id, { orderType: "MARKET" })}
                    className={`rounded-lg py-2 border ${selectedOrder?.orderType === "MARKET" ? "bg-zinc-700 border-zinc-600" : "bg-black border-zinc-800 text-gray-500"}`}
                  >Market</button>
                </div>
              ...
              <div className={`${card} p-5`}>
                <h2 className="text-sm text-gray-400 font-semibold mb-3">Positions</h2>
                <div className="space-y-2 max-h-48 overflow-auto pr-1">
                  {orders.length === 0 && <p className="text-sm text-gray-500">No positions yet.</p>}
                  ...
                  {orders.map((order) => (
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-y-1 text-xs mt-2">
                        <span className="text-gray-500">Entry</span><span className="text-right">{formatPrice(order.entry)}</span>
                        <span className="text-gray-500">Margin</span><span className="text-right">{orderMargin(order).toFixed(2)} USDT</span>
                        <span className="text-gray-500">PnL</span><span className={`text-right ${orderUnrealizedPnl(order) >= 0 ? "text-green-400" : "text-red-400"}`}>{orderUnrealizedPnl(order).toFixed(2)} USDT</span>
                        <span className="text-gray-500">ROI</span><span className={`text-right ${orderRoi(order) >= 0 ? "text-green-400" : "text-red-400"}`}>{orderRoi(order).toFixed(2)}%</span>
                      </div>
                    </button>
                  ))}
                </div>
