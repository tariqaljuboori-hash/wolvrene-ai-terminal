...
type TradeManagerPlan = {
  mode: "NO_POSITION" | "PROTECT" | "HOLD" | "REDUCE" | "DANGER";
  label: string;
  pnl: number;
  roi: number;
  progress: number;
...
export default function WolvreneTerminal() {
  ...
  const tradeManager = useMemo<TradeManagerPlan>(() => {
    const mark = livePrice || lastCandleRef.current?.close || 0;
    ...
    if (!order || !mark) {
      return {
        ...
        roi: 0,
        progress: 0,
        nextAction: "Wait for a confirmed signal or select an active position.",
        beAction: "Breakeven unavailable until a position is open.",
        trailAction: "Trailing stop unavailable until a position is in profit.",
        warning: "No trade management needed right now.",
    ...
    const tp1 = order.tps[0]?.price || null;
    const distanceToTp1 = tp1 ? Math.abs(tp1 - order.entry) : 0;
    const currentProgress = distanceToTp1 ? Math.min(125, Math.max(0, (Math.abs(mark - order.entry) / distanceToTp1) * 100)) : 0;
    const tp1Hit = Boolean(order.tps[0]?.hit);
    const protectedByBE = isLong ? order.sl >= order.entry : order.sl <= order.entry;
    const againstPosition = isLong ? mark < order.entry : mark > order.entry;
    const signalAgainst = signalEngine.direction !== "WAIT" && signalEngine.direction !== order.side;

    let mode: TradeManagerPlan["mode"] = "HOLD";
    let label = "HOLD / MONITOR";
    let nextAction = "Let the setup work. Do not touch the trade unless structure changes.";
    let beAction = protectedByBE ? "SL is already protected near breakeven." : "Move SL to breakeven after TP1 or strong profit confirmation.";
    let trailAction = "Trail only after TP2 or clear continuation momentum.";
    let warning = "No urgent danger detected.";
    let colorClass = "text-yellow-400";

    if (tp1Hit || roi >= 1.2) {
      mode = "PROTECT";
      label = "PROTECT PROFIT";
      nextAction = protectedByBE ? "Keep protection active and let runners continue." : "Move SL to breakeven and secure the position.";
      beAction = protectedByBE ? "Breakeven already active." : "Recommended: move SL to entry now.";
      trailAction = "Start trailing after TP2 or after another clean impulse.";
      colorClass = "text-green-400";
    }

    if ((roi < -0.6 && againstPosition) || signalAgainst) {
      mode = "REDUCE";
      label = "REDUCE RISK";
      nextAction = "Reduce exposure or tighten risk. The live read is not fully supporting this position.";
      beAction = "Do not move SL wider. Keep risk controlled.";
      trailAction = "Trailing is not recommended while the position is under pressure.";
      warning = signalAgainst ? "Signal engine is leaning against the selected position." : "Position is moving against entry.";
      colorClass = "text-orange-400";
    }

    if (roi <= -1.5 || (againstPosition && currentProgress > 65 && signalAgainst)) {
      mode = "DANGER";
      label = "DANGER / DEFEND";
      nextAction = "Defend capital first. Consider closing or cutting size if structure does not recover quickly.";
      beAction = "Breakeven is no longer the priority; risk reduction is.";
      trailAction = "No trailing. Manage downside immediately.";
      warning = "Trade risk is elevated. Do not average down.";
      colorClass = "text-red-400";
    }

    return { mode, label, pnl, roi, progress: Math.round(currentProgress), nextAction, beAction, trailAction, warning, colorClass };
  }, [selectedOrder, livePrice, signalEngine, orders]);
  ...
  function moveOrderStopToBE(order: TradeOrder) {
    updateOrder(order.id, { sl: order.entry });
    addJournal(`Trade Manager moved SL to breakeven for ${order.side} #${String(order.id).slice(-4)}`);
  }

  function applySmartTrail(order: TradeOrder) {
    const mark = livePrice || order.entry;
    const isLong = order.side === "LONG";
    const trailDistance = Math.abs(mark - order.entry) * 0.45 || order.entry * 0.0025;
    const nextSl = isLong ? Math.max(order.sl, mark - trailDistance) : Math.min(order.sl, mark + trailDistance);
    updateOrder(order.id, { sl: nextSl });
    addJournal(`Trade Manager applied smart trailing SL for ${order.side} #${String(order.id).slice(-4)} at ${formatPrice(nextSl)}`);
  }
    ...
    setOrders((prev) =>
      prev.map((order) => {
        ...
        const tps = order.tps.map((tp) => {
          ...
          if (hit) {
            changed = true;
            if (tp.label === "TP1") tp1JustHit = true;
        ...
        if (tp1JustHit) {
          const shouldProtect = isLong ? order.sl < order.entry : order.sl > order.entry;
          if (shouldProtect) {
            addJournal(`Trade Manager auto-protected ${order.side} #${String(order.id).slice(-4)} at breakeven after TP1`);
            return { ...order, sl: order.entry, tps };
          }
        }
  ...
  }, [livePrice, timeframe, session, sessionCountdown, bias, wolfMode, confidence, signalEngine, tradeManager, marketStats, orders, alerts, selectedOrder]);
  ...
  const aiInsights = useMemo(() => {
    ...
    notes.unshift(`Signal Engine: ${signalEngine.display} / ${signalEngine.direction} / ${signalEngine.risk} risk.`);
    if (selected) notes.push(`Trade Manager: ${tradeManager.label} — ${tradeManager.nextAction}`);
  ...
  function buildAIResponse(question: string) {
    ...
    const header = `WOLVRENE LIVE BRAIN\nBTCUSDT ${timeframe} · ${session} · Signal ${signalEngine.display} · Direction ${signalEngine.direction} · Risk ${signalEngine.risk} · Confidence ${signalEngine.confidence}% · Orders ${activeOrders} · Alerts ${activeAlerts}`;

    if (!mark) {
      return `${header}\n\nLive price is not confirmed yet. Wait until Bitget tick is live before trusting entries, TP/SL, or risk numbers.`;
    }

    if (q.includes("signal") || q.includes("اشارة") || q.includes("إشارة") || q.includes("why") || q.includes("ليش")) {
      return `${header}\n\nSignal state: ${signalEngine.display}.\nReason: ${signalEngine.reason}.\nWarning: ${signalEngine.warning}.\nPlan: ${signalEngine.direction === "WAIT" ? "No execution yet." : `Entry ${formatPrice(signalEngine.entry || mark)} · SL ${formatPrice(signalEngine.sl || mark)} · TP1 ${formatPrice(signalEngine.tp1 || mark)}`}`;
    }

    if (q.includes("entry") || q.includes("دخول") || q.includes("long") || q.includes("short")) {
      return `${header}\n\nExecution read: ${signalEngine.direction}.\nCurrent mark: ${formatPrice(mark)}.\nSignal entry: ${signalEngine.entry ? formatPrice(signalEngine.entry) : "WAIT"}.\nBest action: ${signalEngine.state.includes("CONFIRMED") ? "execution is allowed with controlled risk" : signalEngine.state.includes("WATCH") ? "watch only; wait for confirmation" : "wait for structure clarity"}.\nRisk note: ${signalEngine.warning}`;
    }
    ...
    return `${header}\n\nAI read: ${aiInsights[0] || "Waiting for cleaner context."}\n\nSignal reason: ${signalEngine.reason}.\nSignal warning: ${signalEngine.warning}.\nTrade manager: ${tradeManager.nextAction}\nCurrent mark: ${formatPrice(mark)}.\nActive orders: ${activeOrders}. Alerts: ${activeAlerts}.`;
  }
      ...
      <div className={fullscreen ? "w-full max-w-[1800px] mx-auto" : "w-full max-w-7xl mx-auto"}>
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-gray-500 text-xs tracking-wide">BTC AI Dashboard v16.8 — Patch 9 AI Merge + Trade Manager</p>
          </div>
          ...
          <div className="min-w-0">
            ...
            <div className={`${card} p-4 min-w-0 overflow-hidden`}>
              ...
              >
                ...
                {signalEngine.direction !== "WAIT" && signalEngine.entry && signalEngine.sl && signalEngine.tp1 && (
                  <>
                    <LineButton
                      top={priceToTop(signalEngine.entry)}
                      color={signalEngine.direction === "LONG" ? "#22c55e" : "#ef4444"}
                      label={`SIGNAL ${signalEngine.direction} • ${signalEngine.state.replace("_", " ")}`}
                      value={signalEngine.entry}
                      dashed
                      onMouseDown={() => {}}
                      onClick={() => {
                        setAiOpen(true);
                    ...
                    <LineButton
                      top={priceToTop(signalEngine.sl)}
                      color="#ef4444"
                      label="SIGNAL SL"
                      value={signalEngine.sl}
                      dashed
                      onMouseDown={() => {}}
                      onClick={() => {
                        setAiOpen(true);
                    ...
                    <LineButton
                      top={priceToTop(signalEngine.tp1)}
                      color="#22c55e"
                      label="SIGNAL TP1"
                      value={signalEngine.tp1}
                      dashed
                      onMouseDown={() => {}}
                      onClick={() => {
                        setAiOpen(true);
                    ...
                    {signalEngine.tp2 && (
                      <LineButton
                        top={priceToTop(signalEngine.tp2)}
                        color="#14b8a6"
                        label="SIGNAL TP2"
                        value={signalEngine.tp2}
                        dashed
                        onMouseDown={() => {}}
                        onClick={() => {
                          setAiOpen(true);
                    ...
                    {signalEngine.tp3 && (
                      <LineButton
                        top={priceToTop(signalEngine.tp3)}
                        color="#facc15"
                        label="SIGNAL TP3"
                        value={signalEngine.tp3}
                        dashed
                        onMouseDown={() => {}}
                        onClick={() => {
                          setAiOpen(true);
                    ...
                    {priceToTop(signalEngine.entry) !== null && (
                      <div
                        className={`absolute left-4 z-30 max-w-[260px] rounded-xl border px-3 py-2 text-[11px] shadow-2xl backdrop-blur ${
                          signalEngine.direction === "LONG"
                            ? "border-green-500/40 bg-green-500/10 text-green-300"
                            : "border-red-500/40 bg-red-500/10 text-red-300"
                        }`}
                        style={{ top: Math.max(8, Number(priceToTop(signalEngine.entry)) - 46) }}
                      >
                        <div className="font-black tracking-wide">{signalEngine.display}</div>
                        <div className="mt-1 text-gray-300">Entry {formatPrice(signalEngine.entry)} · Risk {signalEngine.risk}</div>
                      </div>
                    )}
                  </>
                )}
            ...
            <aside className="space-y-4 min-w-0">
              <div className={`${card} p-5`}>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-[10px] px-2 py-1 rounded-full border border-yellow-600/40 text-yellow-500 bg-yellow-500/10">V16.8</span>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 mt-4 text-xs">
                  ...
                  <button
                    disabled={signalEngine.direction === "WAIT" || !signalEngine.entry}
                    onClick={() => signalEngine.direction !== "WAIT" && signalEngine.entry && createOrder(signalEngine.direction, signalEngine.entry)}
                    className="rounded-xl border border-zinc-800 bg-black p-3 hover:border-yellow-700 disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    Use Signal
                  </button>
                </div>
              ...
              <div className={`${card} p-5`}>
                <div className="flex items-center justify-between mb-3">
                  <h2 className="text-sm text-gray-400 font-semibold">AI Trade Manager</h2>
                  <span className={`text-[10px] px-2 py-1 rounded-full border border-zinc-700 bg-black/70 ${tradeManager.colorClass}`}>PHASE 6</span>
                </div>
                <p className={`text-2xl font-black ${tradeManager.colorClass}`}>{tradeManager.label}</p>
                <p className="text-[11px] text-gray-500 mt-1">PnL {tradeManager.pnl.toFixed(2)} USDT · ROI {tradeManager.roi.toFixed(2)}% · Progress {tradeManager.progress}%</p>
                <div className="mt-4 rounded-xl border border-zinc-800 bg-black/55 p-3 text-xs space-y-2">
                  <p><span className="text-gray-500">Next:</span> {tradeManager.nextAction}</p>
                  <p><span className="text-gray-500">BE:</span> {tradeManager.beAction}</p>
                  <p><span className="text-gray-500">Trail:</span> {tradeManager.trailAction}</p>
                  <p className="text-orange-400/90"><span className="text-gray-500">Warning:</span> {tradeManager.warning}</p>
                </div>
                <div className="grid grid-cols-3 gap-2 mt-4 text-xs">
                  <button disabled={!selectedOrder} onClick={() => selectedOrder && moveOrderStopToBE(selectedOrder)} className="rounded-xl border border-zinc-800 bg-black p-3 hover:border-green-700 disabled:cursor-not-allowed disabled:opacity-40">Move BE</button>
                  <button disabled={!selectedOrder} onClick={() => selectedOrder && applySmartTrail(selectedOrder)} className="rounded-xl border border-zinc-800 bg-black p-3 hover:border-yellow-700 disabled:cursor-not-allowed disabled:opacity-40">Trail SL</button>
                  <button disabled={!selectedOrder} onClick={() => selectedOrder && closePartial(selectedOrder.id, 50)} className="rounded-xl border border-zinc-800 bg-black p-3 hover:border-orange-700 disabled:cursor-not-allowed disabled:opacity-40">Cut 50%</button>
                </div>
              </div>
