...
// WOLVRENE v32 PRIME TERMINAL — FINAL FULL BUILD
// Full UI polish, cleaner chart lifecycle, liquidity filtering, signal memory, Prime Signal Console, and production-ready dashboard text.
...
export default function WolvreneTerminal() {
  ...
  const visualIntelligence = useMemo(() => {
    ...
    if (decisionPlan.direction && decisionPlan.markerTime && decisionPlan.entry && (decisionPlan.phase === "EXECUTE" || decisionPlan.phase === "VALIDATED")) {
  ...
  useEffect(() => {
    ...
    const maturePhase = plan.phase === "EXECUTE" || plan.phase === "VALIDATED" || (plan.phase === "FILTERED" && Boolean(smartSignalRef.current));
  ...
  const uiMark = livePrice || lastCandleRef.current?.close || v25FinalBrain.entry || 0;
  const uiAvgRange = recentCandles.length >= 20
    ? recentCandles.slice(-20).reduce((sum, c) => sum + Math.max(c.high - c.low, 0), 0) / 20
    : Math.max(uiMark * 0.002, 1);

  const uiPrimaryState =
    v25FinalBrain.action === "ENTER NOW" ? `ENTER ${v25FinalBrain.finalDirection || ""}`.trim() :
    v25FinalBrain.action === "ACTIVE TRADE" ? `ACTIVE ${v25FinalBrain.finalDirection || ""}`.trim() :
    v25FinalBrain.action === "WAIT RETEST" ? `WAIT RETEST ${v25FinalBrain.finalDirection || ""}`.trim() :
    v25FinalBrain.action === "EXIT EARLY" ? "EXIT WATCH" :
    v25FinalBrain.action === "FILTERED" ? "STANDBY" :
    v25FinalBrain.finalDirection ? `WATCH ${v25FinalBrain.finalDirection}` :
    "WAITING FOR SETUP";

  const uiPrimaryTone =
    uiPrimaryState.includes("ENTER") || uiPrimaryState.includes("ACTIVE") ? "text-green-400" :
    uiPrimaryState.includes("EXIT") ? "text-red-400" :
    uiPrimaryState.includes("WAIT") || uiPrimaryState.includes("WATCH") ? "text-yellow-400" :
    "text-gray-300";

  const cleanLiquidityZones = visualIntelligence.zones
    .filter((zone) => !uiMark || Math.abs(uiMark - zone.price) <= uiAvgRange * 10)
    .slice(-2);

  const cleanHeatZones = v23EliteEngine.heatZones
    .filter((zone) => !uiMark || Math.abs(uiMark - zone.price) <= uiAvgRange * 12)
    .slice(-3);

  const cleanVisualMarkers = visualIntelligence.markers
      ...
      <div className={fullscreen ? "w-full max-w-[1800px] mx-auto" : "w-full max-w-7xl mx-auto"}>
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-gray-500 text-xs tracking-wide">BTC AI Terminal — Prime Execution Build</p>
          </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_360px] gap-4">
          <div className="min-w-0">
            ...
            <div className={`${card} p-4 min-w-0 overflow-hidden`}>
              ...
              >
                <div ref={chartRef} className="w-full min-w-0 h-[560px]" />

                <div className="absolute left-3 top-3 z-40 rounded-xl border border-yellow-500/20 bg-black/65 px-3 py-2 backdrop-blur-md shadow-[0_0_24px_rgba(0,0,0,0.65)] pointer-events-none">
                  <div className="flex items-center gap-3">
                    <span className="text-[10px] font-black tracking-[0.18em] text-yellow-400">WOLVRENE PRIME</span>
                    <span className={`rounded-full px-2 py-0.5 text-[9px] font-black ${uiPrimaryState.includes("ENTER") || uiPrimaryState.includes("ACTIVE") ? "bg-green-500/20 text-green-300" : uiPrimaryState.includes("EXIT") ? "bg-red-500/20 text-red-300" : "bg-zinc-800 text-gray-300"}`}>{uiPrimaryState}</span>
                    <span className="text-[10px] text-gray-500">{v25FinalBrain.masterScore}% · {v25FinalBrain.entryQuality}</span>
                  </div>
                </div>
            ...
            <aside className="space-y-4 min-w-0">
              <div className={`${card} p-5`}>
                <div className="flex items-center justify-between mb-3">
                  <h2 className="text-sm text-gray-400 font-semibold">Prime Signal Console</h2>
                  <span className="text-[10px] px-2 py-1 rounded-full border border-yellow-600/40 text-yellow-500 bg-yellow-500/10">PRIME</span>
                </div>

                <p
                  className={`text-3xl font-black ${uiPrimaryTone}`}
                >{uiPrimaryState}</p>
