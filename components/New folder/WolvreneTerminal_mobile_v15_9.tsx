...
export default function WolvreneTerminal() {
  ...
  useEffect(() => {
    ...
    const resizeObserver = new ResizeObserver(() => {
      ...
      requestAnimationFrame(() => {
        if (!chartAliveRef.current || !chartRef.current || !chartApiRef.current) return;
        const nextHeight = isMobile ? (fullscreen ? Math.max(window.innerHeight - 140, 420) : 360) : fullscreen ? 720 : 520;
        chartApiRef.current.applyOptions({
          width: Math.max(chartRef.current.clientWidth || window.innerWidth - 48, 320),
          height: nextHeight,
        });
      });
    });
  ...
  }, [timeframe, settings, fullscreen, isMobile, reloadCandles]);
  ...
  return (
    <main
      ...
      className="min-h-screen bg-[radial-gradient(circle_at_top,#18120a_0%,#070707_42%,#000_100%)] text-white p-3 md:p-4 overflow-x-hidden"
    >
      <div className={fullscreen ? "w-full max-w-[1800px] mx-auto" : "w-full max-w-7xl mx-auto"}>
        <div className="flex items-center justify-between gap-3 mb-5">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <img src="/wolvrene-logo.png" alt="Wolvrene Logo" className="w-9 h-9 sm:w-11 sm:h-11 object-contain" />
              <h1
                className="text-2xl sm:text-3xl font-black tracking-[0.08em]"
                style={{ color: gold, textShadow: "0 0 16px rgba(216,155,0,0.55)" }}
              >
                WOLVRENE
              </h1>
            </div>
            <p className="text-gray-500 text-xs tracking-wide">BTC AI Dashboard v15.9 — Mobile Ready</p>
          </div>
          ...
          <div className="min-w-0">
            ...
            <div className={`${card} p-4 min-w-0 overflow-hidden`}>
              ...
              <div
                ...
                className="relative overflow-hidden rounded-xl border border-zinc-800 bg-black w-full min-w-0 min-h-[360px] md:min-h-[520px]"
              >
                <div ref={chartRef} className="w-full min-w-0 h-[360px] md:h-[520px]" />
