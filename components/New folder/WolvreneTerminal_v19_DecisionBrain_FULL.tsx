...
type DecisionPhase = "SCANNING" | "SPAWNED" | "VALIDATED" | "EXECUTE" | "MANAGE" | "CANCELLED" | "NO_TRADE";
...
const defaultDecisionSettings: DecisionSettings = { enabled: true, holdBars: 6, executeConfidence: 82, validateConfidence: 68, spawnConfidence: 54, cancelOnOppositeShift: true, showDecisionPanel: true };
...
export default function WolvreneTerminal() {
  ...
  const rawDecisionPlan = useMemo<DecisionPlan>(() => {
    ...
    const direction = signalPlan.direction || structureDirection;
    ...
    const quality = Math.round(Math.max(0, Math.min(100, signalPlan.confidence + structureState.score + triggerScore + agreementBoost + volatilityAdjust - 24)));
    ...
    const phase: DecisionPhase = !decisionSettings.enabled
      ...
      ? "CANCELLED"
      : quality >= decisionSettings.executeConfidence && triggerScore >= 12
      ? "EXECUTE"
    ...
    const action = phase === "EXECUTE"
      ? `EXECUTE ${direction} only with risk control.`
      : phase === "VALIDATED"
      ? `Validated ${direction} setup. Wait for trigger candle or retest.`
      : phase === "SPAWNED"
    ...
    const reason = [structureState.summary, signalPlan.reason, directionalAgreement ? "Engine + structure aligned." : "Waiting for stronger alignment."].filter(Boolean).join(" ");
  ...
  }, [livePrice, signalPlan, structureState, candlesSummary.volatility, decisionSettings, timeframe]);
  ...
  useEffect(() => {
    ...
    setDecisionSettings(loadJson("wolvreneDecisionSettingsV1", defaultDecisionSettings));
  ...
  function buildCompactDiscordSignal(label = "WOLVRENE SIGNAL") {
    ...
    return [
      ...
      `**${status}** · ${direction} · Q:${decisionPlan.quality}% · C:${signalPlan.confidence}%`,
      ...
      `Structure: ${decisionPlan.structure} · Trigger: ${decisionPlan.trigger}`,
      `Plan: ${planLine}`,
    ].join("\n");
  }
  ...
  }, [livePrice, timeframe, session, sessionCountdown, bias, wolfMode, confidence, signalPlan, decisionPlan, structureState, marketStats, candlesSummary, backtestStats, learningWeights, orders, alerts, selectedOrder]);
      ...
      <div className={fullscreen ? "w-full max-w-[1800px] mx-auto" : "w-full max-w-7xl mx-auto"}>
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-gray-500 text-xs tracking-wide">BTC AI Dashboard v19.0 — Decision Brain</p>
          </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="rounded-2xl border border-zinc-800 bg-black/70 p-4 text-sm space-y-3">
                          ...
                          <div className="rounded-xl border border-yellow-700/25 bg-yellow-500/5 p-3 space-y-2">
                            <p className="text-[11px] font-black text-yellow-500">v19 Decision Brain Settings</p>
