...
// WOLVRENE v20 Intelligence Core FINAL
// Built from v19 Decision Brain: adds liquidity logic, trigger validation, Smart Management, Discord Pro Signals, and richer AI context.
...
export default function WolvreneTerminal() {
  ...
  function buildCompactDiscordSignal(label = "WOLVRENE SIGNAL") {
    ...
    return [
      ...
      `**${status}** · ${direction} · Q:${decisionPlan.quality}% · C:${signalPlan.confidence}% · ${decisionPlan.proSignal ? "PRO" : "WATCH"}`,
  ...
  }, [livePrice, timeframe, session, sessionCountdown, bias, wolfMode, confidence, signalPlan, decisionPlan, structureState, liquidityState, triggerValidation, managementBrain, marketStats, candlesSummary, backtestStats, learningWeights, orders, alerts, selectedOrder]);
  ...
  useEffect(() => {
    ...
    if (!decisionPlan.direction || !(decisionPlan.phase === "VALIDATED" || decisionPlan.phase === "EXECUTE")) return;
    if (decisionPlan.quality < externalAlertSettings.minSignalConfidence) return;

    const key = `${timeframe}-${decisionPlan.phase}-${decisionPlan.direction}-${decisionPlan.markerTime || "live"}`;
      ...
      <div className={fullscreen ? "w-full max-w-[1800px] mx-auto" : "w-full max-w-7xl mx-auto"}>
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-gray-500 text-xs tracking-wide">BTC AI Dashboard v20.0 — Intelligence Core</p>
          </div>
