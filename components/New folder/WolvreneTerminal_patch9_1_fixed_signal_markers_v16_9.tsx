...
export default function WolvreneTerminal() {
  ...
  const signalPlan = useMemo<SignalPlan>(() => {
    ...
    const score = Math.round(Math.max(24, Math.min(93, 48 + sessionBoost + tfBoost + momentumBoost + locationBoost + openOrderPenalty)));
  ...
  }, [livePrice, timeframe, session, bias, marketStats.high, marketStats.low, orders]);
  ...
  }, [livePrice, timeframe, session, sessionCountdown, bias, wolfMode, confidence, signalPlan, marketStats, orders, alerts, selectedOrder]);
  ...
  function buildAIResponse(question: string) {
    ...
    const header = `WOLVRENE LIVE BRAIN\nBTCUSDT ${timeframe} · ${session} · Signal ${signalPlan.state} · Risk ${signalPlan.risk} · Confidence ${signalPlan.confidence}% · Orders ${activeOrders} · Alerts ${activeAlerts}`;
