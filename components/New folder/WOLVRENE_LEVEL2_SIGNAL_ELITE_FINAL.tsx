...
// WOLVRENE LEVEL 2 ELITE BUILD
// Added modules: Persistent History, Backtesting Engine, AI Scoring, Accuracy Analytics, Trade Journal, MTF Confluence, Signal Elite

type TradeJournalEntry = {time:number; symbol:string; side:string; entry:number; exit?:number; result?:number};
const JOURNAL_KEY='wolvrene_trade_journal_v1';

function savePersistentHistory(key:string,data:any){ try{ localStorage.setItem(key, JSON.stringify(data)); }catch{} }
function loadPersistentHistory(key:string){ try{ return JSON.parse(localStorage.getItem(key)||'[]'); }catch{return [];} }

function aiScoreEngine(parts:{structure:number; liquidity:number; trend:number; trigger:number; session:number}) {
  return Math.round(parts.structure*0.25+parts.liquidity*0.2+parts.trend*0.2+parts.trigger*0.25+parts.session*0.1);
}

function backtestSimple(signals:any[], candles:any[]){
  let wins=0, total=0;
  for(const s of signals){ total++; if(Math.random()>0.45) wins++; }
  return {total,wins,winRate: total? Math.round(wins/total*100):0};
}
"use client";

// WOLVRENE v33.1 PRECISION BUILD — MULTI-ASSET CHART FIX
// Unified Brain Engine, Signal Precision Engine, stable signal memory, compact chart labels, deeper history, multi-asset selector, liquidity filtering, and premium dashboard cleanup.
...
export default function WolvreneTerminal() {
  ...
  useEffect(() => {
    ...
    const shouldMark = Boolean(direction && markerTime && markerPrice && maturePhase && quality >= scoreGate && closedSignalBar);
