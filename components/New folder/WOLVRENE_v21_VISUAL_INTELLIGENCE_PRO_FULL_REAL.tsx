...
// WOLVRENE v21 Visual Intelligence Pro FULL
// Real upgrade: Smart Signal Control, lifecycle replacement, visual structure labels,
// BOS/CHoCH, liquidity sweeps, trigger labels, Decision Brain Pro actions,
// Discord Elite filtering, and AI visual reasoning.
...
type DecisionPhase = "SCANNING" | "SPAWNED" | "VALIDATED" | "EXECUTE" | "MANAGE" | "EXIT" | "CANCELLED" | "NO_TRADE";
...
const defaultExternalAlertSettings: ExternalAlertSettings = { discordWebhook: "", telegramWebhook: "", emailWebhook: "", enabled: false, discordSignalOnly: true, autoDiscordSignals: false, minSignalConfidence: 82 };
const defaultDecisionSettings: DecisionSettings = { enabled: true, holdBars: 8, executeConfidence: 82, validateConfidence: 68, spawnConfidence: 54, cancelOnOppositeShift: true, showDecisionPanel: true, requireTriggerForExecute: true, proSignalOnly: true };
...
export default function WolvreneTerminal() {
  ...
  const rawDecisionPlan = useMemo<DecisionPlan>(() => {
    ...
    const direction = triggerValidation.direction || signalPlan.direction || structureDirection;
    const directionalAgreement = Boolean(direction && signalPlan.direction && structureDirection && signalPlan.direction === structureDirection);
    ...
    const agreementBoost = directionalAgreement ? 10 : structureDirection && signalPlan.direction && structureDirection !== signalPlan.direction ? -12 : 0;
    const liquidityBoost = liquidityState.trapDirection && direction === liquidityState.trapDirection ? liquidityState.score : Math.max(0, liquidityState.score - 8);
    const triggerBoost = triggerValidation.direction && direction === triggerValidation.direction ? triggerValidation.score : Math.max(0, triggerValidation.score - 10);
    const volatilityAdjust = candlesSummary.volatility === "HIGH" ? -4 : candlesSummary.volatility === "LOW" ? -2 : 2;
    const proSignal = Boolean(direction && triggerValidation.quality !== "NONE" && (directionalAgreement || liquidityState.trapDirection === direction || triggerValidation.quality === "SNIPER"));
    const quality = Math.round(Math.max(0, Math.min(100, signalPlan.confidence + structureState.score + triggerScore + liquidityBoost + triggerBoost + agreementBoost + volatilityAdjust - 32)));
    ...
    const phase: DecisionPhase = !decisionSettings.enabled
      ...
      ? "CANCELLED"
      : quality >= decisionSettings.executeConfidence && (!decisionSettings.requireTriggerForExecute || triggerValidation.quality === "VALID" || triggerValidation.quality === "STRONG" || triggerValidation.quality === "SNIPER") && (!decisionSettings.proSignalOnly || proSignal)
      ? "EXECUTE"
      : quality >= decisionSettings.validateConfidence
      ? "VALIDATED"
      : quality >= decisionSettings.spawnConfidence
      ? "SPAWNED"
      : "NO_TRADE";
    ...
    const action = phase === "EXECUTE"
      ? `EXECUTE ${direction}: ${triggerValidation.summary}. Use pro-signal risk control.`
      : phase === "VALIDATED"
      ? `Validated ${direction} setup. Wait for retest or continuation after ${triggerValidation.quality} trigger.`
      : phase === "SPAWNED"
      ? `Early ${direction} idea spawned. Watch confirmation.`
      : phase === "MANAGE"
      ? `Manage active ${direction} decision. Protect invalidation.`
      : phase === "CANCELLED"
      ? "Decision cancelled by opposite structure shift."
      : "Scan only. No mature decision yet.";

    const reason = [structureState.summary, liquidityState.summary, triggerValidation.summary, signalPlan.reason, proSignal ? "Pro signal conditions detected." : "Waiting for stronger alignment."].filter(Boolean).join(" ");
    ...
    return {
      ...
      shouldMark: phase === "SPAWNED" || phase === "VALIDATED" || phase === "EXECUTE",
    };
  }, [livePrice, signalPlan, structureState, liquidityState, triggerValidation, candlesSummary.volatility, decisionSettings, timeframe]);
  ...
  useEffect(() => {
    setActiveDecision((prev) => {
      ...
      if (rawDecisionPlan.phase === "CANCELLED" || invalidated || expired) {
        const cancelled: DecisionPlan = { ...(prev || rawDecisionPlan), phase: "CANCELLED", action: invalidated ? "Decision invalidated by price crossing invalidation." : expired ? "Decision expired. Waiting for a fresh trigger." : rawDecisionPlan.action, createdAt: prev?.createdAt || Date.now() };
        setDecisionHistory((history) => [cancelled, ...history].slice(0, 80));
        return cancelled;
      }
      ...
      if (rawDecisionPlan.shouldMark && rawDecisionPlan.direction) {
        ...
        if (!sameDirection || stronger || prev.phase === "CANCELLED" || prev.phase === "NO_TRADE") {
      ...
      if (prev && prev.phase !== "CANCELLED" && prev.phase !== "NO_TRADE") {
  ...
  const visualIntelligence = useMemo(() => {
    ...
    if (sample.length < 12) {
      return {
        ...
        lastDecisionAction: "WAIT" as "ENTER NOW" | "WAIT RETEST" | "EXIT EARLY" | "CANCELLED" | "MANAGE RUNNER" | "WAIT",
      };
    }
    ...
    const lastDecisionAction =
      ...
      decisionPlan.phase === "CANCELLED" ? "CANCELLED" :
    ...
    if (decisionPlan.direction && decisionPlan.markerTime && decisionPlan.entry && (decisionPlan.phase === "EXECUTE" || decisionPlan.phase === "VALIDATED" || decisionPlan.phase === "CANCELLED")) {
    ...
    return {
      markers: markers.slice(-28),
      zones: zones.slice(-4),
      summary: `${currentStructure} path · ${markers.filter((m) => m.kind !== "STRUCTURE").slice(-3).map((m) => m.label).join(" / ") || "waiting"}`,
      currentStructure,
      lastDecisionAction,
    };
  }, [recentCandles, triggerValidation, decisionPlan.id, decisionPlan.phase, decisionPlan.direction, decisionPlan.markerTime, decisionPlan.entry, decisionPlan.quality, managementBrain.action]);
  ...
  function buildCompactDiscordSignal(label = "WOLVRENE SIGNAL") {
    const mark = livePrice || lastCandleRef.current?.close || signalPlan.entry || 0;
    const direction = decisionPlan.direction || signalPlan.direction || "WAIT";
    const status = decisionPlan.phase === "EXECUTE" || decisionPlan.phase === "MANAGE" ? decisionPlan.phase : signalPlan.state;
    ...
    return [
      ...
      `**${status}** · ${direction} · Q:${decisionPlan.quality}% · C:${signalPlan.confidence}% · ${decisionPlan.proSignal ? "ELITE" : "WATCH"}`,
      `Decision: ${visualIntelligence.lastDecisionAction} · Visual: ${visualIntelligence.summary}`,
  ...
  function sendDiscordSignalNow() {
    if (!decisionPlan.direction || decisionPlan.phase === "NO_TRADE" || decisionPlan.phase === "SCANNING" || decisionPlan.phase === "CANCELLED") {
    ...
    if (decisionPlan.quality < externalAlertSettings.minSignalConfidence) {
      addJournal(`Discord signal skipped: quality ${decisionPlan.quality}% below minimum ${externalAlertSettings.minSignalConfidence}%`);
      return;
    }
  ...
  }, [livePrice, timeframe, session, sessionCountdown, bias, wolfMode, confidence, signalPlan, decisionPlan, structureState, liquidityState, triggerValidation, visualIntelligence, managementBrain, marketStats, candlesSummary, backtestStats, learningWeights, orders, alerts, selectedOrder]);
  ...
  useEffect(() => {
    ...
    const maturePhase = plan.phase === "EXECUTE" || plan.phase === "VALIDATED" || plan.phase === "CANCELLED";
    const shouldMark = Boolean(direction && markerTime && markerPrice && (maturePhase || signalPlan.shouldMark));
    ...
    const stronger = !prevSmart || quality >= prevSmart.quality + 4 || plan.phase === "EXECUTE" || oppositeDirection;

    if (sameDirection && inCooldown && !stronger) return;

    const phaseLabel = plan.phase === "EXECUTE" ? "ENTER NOW" : plan.phase === "VALIDATED" ? "WAIT RETEST" : plan.phase === "CANCELLED" ? "CANCELLED" : signalPlan.state;
    ...
    setSignalMarkers((prev) => {
      ...
      return [...cleaned, nextMarker].slice(-24);
    });
  }, [decisionPlan.id, decisionPlan.phase, decisionPlan.direction, decisionPlan.markerTime, decisionPlan.entry, decisionPlan.quality, signalPlan.state, signalPlan.markerTime, signalPlan.markerPrice, signalPlan.shouldMark, signalPlan.direction, signalPlan.confidence, timeframe]);
  ...
  useEffect(() => {
    ...
    if (decisionPlan.quality < Math.max(82, externalAlertSettings.minSignalConfidence)) return;
  ...
  }, [externalAlertSettings.enabled, externalAlertSettings.autoDiscordSignals, externalAlertSettings.discordWebhook, externalAlertSettings.minSignalConfidence, decisionPlan.phase, decisionPlan.direction, decisionPlan.quality, decisionPlan.markerTime, decisionPlan.entry, timeframe]);
      ...
      <div className={fullscreen ? "w-full max-w-[1800px] mx-auto" : "w-full max-w-7xl mx-auto"}>
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-gray-500 text-xs tracking-wide">BTC AI Dashboard v21.0 — Visual Intelligence Pro</p>
          </div>
          ...
          <div className="min-w-0">
            ...
            <div className={`${card} p-4 min-w-0 overflow-hidden`}>
              ...
              >
                ...
                <div className="absolute left-3 top-3 z-40 max-w-[360px] rounded-2xl border border-yellow-500/20 bg-black/70 p-3 backdrop-blur-md shadow-[0_0_30px_rgba(0,0,0,0.65)] pointer-events-none">
                  <div className="flex items-center justify-between gap-3">
                    <p className="text-[10px] font-black tracking-[0.18em] text-yellow-400">v21 VISUAL INTELLIGENCE</p>
                    <span className={`rounded-full px-2 py-0.5 text-[9px] font-black ${visualIntelligence.lastDecisionAction === "ENTER NOW" ? "bg-green-500/20 text-green-300" : visualIntelligence.lastDecisionAction === "CANCELLED" || visualIntelligence.lastDecisionAction === "EXIT EARLY" ? "bg-red-500/20 text-red-300" : "bg-zinc-800 text-gray-300"}`}>{visualIntelligence.lastDecisionAction}</span>
                  </div>
                  <p className="mt-2 text-[11px] text-gray-300">{visualIntelligence.summary}</p>
                  <p className="mt-1 text-[10px] text-gray-500">Smart Control: one active signal · cooldown replace · strongest priority only</p>
                </div>
                ...
                {visualIntelligence.markers.map((marker) => {
                  ...
                  return (
                    <div
                      ...
                      style={{ left: Math.max(4, left - 36), top: bullish ? top + 18 : top - 54 }}
                    >
                      <div className={`rounded-lg border px-2 py-1 text-[9px] font-black shadow-[0_0_16px_rgba(0,0,0,0.85)] ${bullish ? "border-green-400/60 bg-green-500/15 text-green-200" : "border-red-400/60 bg-red-500/15 text-red-200"}`}>
              ...
              <div className={`${card} p-5`}>
                ...
                {decisionSettings.showDecisionPanel && (
                  <div className="mt-4 rounded-2xl border border-yellow-700/30 bg-yellow-500/[0.045] p-3 text-xs space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-yellow-500 font-black">v19 DECISION BRAIN</span>
                      <span className={`rounded-full px-2 py-0.5 text-[10px] font-black ${decisionPlan.phase === "EXECUTE" ? "bg-green-500/20 text-green-300" : decisionPlan.phase === "CANCELLED" ? "bg-red-500/20 text-red-300" : "bg-zinc-800 text-gray-300"}`}>{decisionPlan.phase}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                ...
                <div className="grid grid-cols-2 gap-2 mt-4 text-xs">
                  ...
                  <button
                    ...
                    disabled={!decisionPlan.direction || decisionPlan.phase === "NO_TRADE" || decisionPlan.phase === "SCANNING" || decisionPlan.phase === "CANCELLED"}
                  ...
                  <button
                    ...
                    disabled={!externalAlertSettings.enabled || !externalAlertSettings.discordWebhook || !decisionPlan.direction || decisionPlan.phase === "NO_TRADE" || decisionPlan.phase === "SCANNING" || decisionPlan.phase === "CANCELLED"}
