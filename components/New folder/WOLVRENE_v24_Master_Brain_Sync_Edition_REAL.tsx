...
// WOLVRENE v24 Master Brain Sync Edition
// Real upgrade over v22: label spacing engine, trigger/decision sync,
// sniper-only execution gate, probability heat zones, session sniper score,
// live win-streak tracking, cleaner visual lifecycle, and tighter elite Discord logic.
...
export default function WolvreneTerminal() {
  ...
  const rawDecisionPlan = useMemo<DecisionPlan>(() => {
    ...
    const action = phase === "EXECUTE"
      ...
      : phase === "CANCELLED"
      ? `CANCELLED: ${institutionalPrecision.reason}`
    ...
    return {
      ...
      phase,
      direction,
      confidence: v24MasterBrain.confidence,
      quality,
  ...
  useEffect(() => {
    setActiveDecision((prev) => {
      ...
      if (rawDecisionPlan.phase === "CANCELLED" || invalidated || expired) {
        ...
        setDecisionHistory((history) => history[0]?.phase === "CANCELLED" && history[0]?.direction === cancelled.direction ? history : [cancelled, ...history].slice(0, 50));
        return cancelled;
      }
  ...
  const v23EliteEngine = useMemo(() => {
    ...
    const summary = `v23 ${sniperGrade} · Sniper ${sniperScore}% · Trigger ${triggerSyncStatus} · Session ${sessionSniperScore}`;
  ...
  const v24MasterBrain = useMemo(() => {
    ...
    const masterScore = Math.max(0, Math.min(100, Math.round(
      decisionPlan.quality * 0.34 +
      institutionalPrecision.precisionScore * 0.26 +
      v23EliteEngine.sniperScore * 0.22 +
      signalPlan.confidence * 0.10 +
      (triggerAligned ? 8 : -12) +
      (structureAligned ? 6 : -18)
    )));
    ...
    const phase =
      institutionalPrecision.hardConflict || decisionPlan.phase === "CANCELLED"
        ? "CANCELLED"
    ...
    return {
      ...
      confidence: Math.max(signalPlan.confidence, masterScore),
      stateColorClass: finalDirection === "LONG" ? "text-green-400" : finalDirection === "SHORT" ? "text-red-400" : action === "CANCELLED" ? "text-gray-400" : "text-yellow-400",
      barColorClass: finalDirection === "LONG" ? "bg-green-500" : finalDirection === "SHORT" ? "bg-red-500" : "bg-yellow-500",
      directionColorClass: finalDirection === "LONG" ? "font-bold text-green-400" : finalDirection === "SHORT" ? "font-bold text-red-400" : "font-bold text-gray-400",
      summary: `v24 Master ${masterGrade} · ${action} · ${finalDirection || "WAIT"} · ${masterScore}% · votes L:${longVotes}/S:${shortVotes}`,
    };
  }, [decisionPlan, institutionalPrecision, triggerValidation.direction, triggerValidation.quality, liquidityState.trapDirection, structureState.event, structureState.bias, signalPlan, v23EliteEngine.sniperScore, v23EliteEngine.sniperAllowed, managementBrain.action]);
  ...
  function buildCompactDiscordSignal(label = "WOLVRENE SIGNAL") {
    const mark = livePrice || lastCandleRef.current?.close || signalPlan.entry || 0;
    const direction = v24MasterBrain.finalDirection || decisionPlan.direction || signalPlan.direction || "WAIT";
    const status = v24MasterBrain.consoleState;
    ...
    return [
      ...
      `**${status}** · ${direction} · Master:${v24MasterBrain.masterScore}% · Grade:${v24MasterBrain.masterGrade} · ${decisionPlan.proSignal ? "ELITE" : "WATCH"}`,
      `Decision: ${v24MasterBrain.action} · v24: ${v24MasterBrain.masterGrade} ${v24MasterBrain.masterScore}% · Trigger: ${v24MasterBrain.triggerAligned ? "SYNCED" : "WAIT"}`,
        ...
        targets.map((target) => {
          ...
          return fetch(target.url!, {
            ...
            body: JSON.stringify({
              ...
              timeframe,
              session,
              signal: v24MasterBrain.consoleState,
  ...
  function useSignalPlan() {
    ...
    addJournal(`Signal used: ${v24MasterBrain.consoleState} at ${formatPrice(signalPlan.entry)} — risk ${signalPlan.risk}`);
  ...
  }, [livePrice, timeframe, session, sessionCountdown, bias, wolfMode, confidence, signalPlan, decisionPlan, structureState, liquidityState, triggerValidation, visualIntelligence, institutionalPrecision, v23EliteEngine, v24MasterBrain, managementBrain, marketStats, candlesSummary, backtestStats, learningWeights, orders, alerts, selectedOrder]);
  ...
  const aiInsights = useMemo(() => {
    ...
    notes.push(`v23 Elite Engine: ${v23EliteEngine.summary}. Win streak: ${v23EliteEngine.winStreak}. Sniper allowed: ${v23EliteEngine.sniperAllowed ? "YES" : "NO"}.`);
    notes.push(`v24 Master Brain: ${v24MasterBrain.summary}. Reason: ${v24MasterBrain.reason}`);
  ...
  function buildAIResponse(question: string) {
    ...
    if (q.includes("entry") || q.includes("دخول") || q.includes("long") || q.includes("short") || q.includes("signal")) {
      return `${header}\n\nSignal: ${signalPlan.state}.\nEntry: ${v24MasterBrain.entry ? formatPrice(v24MasterBrain.entry) : "--"}\nSL: ${v24MasterBrain.sl ? formatPrice(v24MasterBrain.sl) : "--"}\nTP1: ${v24MasterBrain.tp1 ? formatPrice(v24MasterBrain.tp1) : "--"}\nTP2: ${v24MasterBrain.tp2 ? formatPrice(v24MasterBrain.tp2) : "--"}\nTP3: ${v24MasterBrain.tp3 ? formatPrice(v24MasterBrain.tp3) : "--"}\n\nReason: ${signalPlan.reason}\nWarning: ${signalPlan.warning || "Manage risk. Do not chase."}`;
    }
  ...
  useEffect(() => {
    ...
    const maturePhase = plan.phase === "EXECUTE" || plan.phase === "VALIDATED" || (plan.phase === "CANCELLED" && Boolean(smartSignalRef.current));
    ...
    setSignalMarkers((prev) => {
      ...
      return [...cleaned, nextMarker].slice(-14);
    });
  ...
  useEffect(() => {
    ...
    if (decisionPlan.quality < Math.max(88, externalAlertSettings.minSignalConfidence) || v23EliteEngine.sniperScore < 92 || v24MasterBrain.masterScore < 88) return;
              ...
              >
                ...
                <div className="absolute left-3 top-3 z-40 max-w-[360px] rounded-2xl border border-yellow-500/20 bg-black/70 p-3 backdrop-blur-md shadow-[0_0_30px_rgba(0,0,0,0.65)] pointer-events-none">
                  <div className="flex items-center justify-between gap-3">
                    <p className="text-[10px] font-black tracking-[0.18em] text-yellow-400">v24 MASTER BRAIN SYNC</p>
                    <span className={`rounded-full px-2 py-0.5 text-[9px] font-black ${visualIntelligence.lastDecisionAction === "ENTER NOW" ? "bg-green-500/20 text-green-300" : visualIntelligence.lastDecisionAction === "CANCELLED" || visualIntelligence.lastDecisionAction === "EXIT EARLY" ? "bg-red-500/20 text-red-300" : "bg-zinc-800 text-gray-300"}`}>{v24MasterBrain.action}</span>
                  </div>
                  <p className="mt-2 text-[11px] text-gray-300">{v24MasterBrain.summary}</p>
                  <p className="mt-1 text-[10px] text-gray-500">{visualIntelligence.summary}</p>
                  <p className="mt-1 text-[10px] text-gray-400">{v24MasterBrain.reason}</p>
                  <p className="mt-1 text-[10px] text-gray-500">One master decision · synced panels · conflict lock · trigger hierarchy</p>
                </div>
                ...
                {visualIntelligence.markers.map((marker) => {
                  ...
                  return (
                    <div
                      ...
                      style={{ left: Math.max(4, left - 36 + (v23EliteEngine.markerOffsets[marker.id]?.x || 0)), top: top + (v23EliteEngine.markerOffsets[marker.id]?.y || (bullish ? 18 : -54)) }}
                    >
            ...
            <aside className="space-y-4 min-w-0">
              <div className={`${card} p-5`}>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-[10px] px-2 py-1 rounded-full border border-yellow-600/40 text-yellow-500 bg-yellow-500/10">V24</span>
                </div>

                <p
                  className={`text-3xl font-black ${v24MasterBrain.stateColorClass}`}
                >
                  {signalPlan.state}
                </p>
                ...
                <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
                  <div className="rounded-xl border border-zinc-800 bg-black/60 p-3">
                    ...
                    <p className="font-bold text-yellow-400">{v24MasterBrain.risk}</p>
                  </div>
                  <div className="rounded-xl border border-zinc-800 bg-black/60 p-3">
                    <p className="text-gray-500">Direction</p>
                    <p className={v24MasterBrain.directionColorClass}>
                      {v24MasterBrain.finalDirection || "WAIT"}
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-5">
                  <div className="flex justify-between text-xs text-gray-500 mb-2">
                    ...
                    <span>{v24MasterBrain.confidence}%</span>
                  </div>
                  <div className="h-3 rounded-full bg-black border border-zinc-800 overflow-hidden">
                    <div
                      className={`h-full ${v24MasterBrain.barColorClass}`}
                      style={{ width: `${v24MasterBrain.confidence}%` }}
                    />
                  </div>
                </div>
                ...
                <p className="text-xs text-gray-400 mt-4">{v24MasterBrain.reason}</p>
