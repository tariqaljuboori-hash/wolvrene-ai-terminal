...
export default function WolvreneTerminal() {
  ...
  const [aiMessages, setAiMessages] = useState<AIMessage[]>([
    {
      ...
      role: "assistant",
      time: "Ready",
      text: "WOLVRENE AI Command Window armed. I can read the current dashboard context, open orders, active alerts, session, bias, and timeframe.",
    },
  ]);
  ...
  function buildAIResponse(question: string) {
    ...
    const header = `BTCUSDT ${timeframe} · ${session} · Bias ${bias} · Confidence ${confidence}%`;
  ...
  function sendAIMessage(text?: string) {
    ...
    const assistantMessage: AIMessage = {
      id: Date.now() + 1,
      role: "assistant",
      text: buildAIResponse(question),
      time: now,
    };

    setAiMessages((prev) => [...prev, userMessage, assistantMessage].slice(-30));
    setAiInput("");
    setAiOpen(true);
    setAiTab("chat");
  }
      ...
      <div className={fullscreen ? "w-full max-w-[1800px] mx-auto" : "w-full max-w-7xl mx-auto"}>
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-gray-500 text-xs tracking-wide">BTC AI Dashboard v16.0 — Patch 4 Foundation Core</p>
          </div>
            ...
            <aside className="space-y-4 min-w-0">
              <div className={`${card} p-5`}>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-[10px] px-2 py-1 rounded-full border border-yellow-600/40 text-yellow-500 bg-yellow-500/10">V16.1</span>
                </div>
                ...
                <p className="text-xs text-gray-600 mt-4">AI Command Window reads live dashboard context, selected order, alerts, session, and timeframe.</p>
        ...
        {aiOpen && (
          <div className="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-[#111116] border border-[#302817] rounded-2xl w-full max-w-5xl max-h-[88vh] overflow-hidden shadow-[0_0_55px_rgba(216,155,0,0.15)]">
              <div className="flex items-center justify-between border-b border-zinc-800 px-5 py-4">
                <div>
                  <h2 className="text-2xl font-black tracking-wide" style={{ color: gold, textShadow: "0 0 14px rgba(216,155,0,0.45)" }}>
                    WOLVRENE AI COMMAND
                  </h2>
                  <p className="text-xs text-gray-500">Live context analyst · {timeframe} · {session} · {bias}</p>
                </div>
                <button onClick={() => setAiOpen(false)} className="text-2xl text-gray-400 hover:text-white">×</button>
              </div>

              <div className="grid lg:grid-cols-[1fr_320px] max-h-[calc(88vh-72px)] overflow-hidden">
                <div className="min-w-0 flex flex-col">
                  ...
                  <div className="flex-1 overflow-auto p-4">
                    ...
                    {aiTab === "context" && (
                      <div className="rounded-2xl border border-zinc-800 bg-black/70 p-4 text-sm space-y-2">
                        <div className="flex justify-between"><span className="text-gray-500">Symbol</span><span>BTCUSDT</span></div>
                        <div className="flex justify-between"><span className="text-gray-500">Timeframe</span><span>{timeframe}</span></div>
                        <div className="flex justify-between"><span className="text-gray-500">Live Price</span><span>{aiContext.mark ? formatPrice(aiContext.mark) : "Waiting"}</span></div>
                        <div className="flex justify-between"><span className="text-gray-500">Session</span><span>{session}</span></div>
                        <div className="flex justify-between"><span className="text-gray-500">Bias</span><span>{bias}</span></div>
                        <div className="flex justify-between"><span className="text-gray-500">Confidence</span><span>{confidence}%</span></div>
                        <div className="flex justify-between"><span className="text-gray-500">Orders</span><span>{orders.length}</span></div>
                        <div className="flex justify-between"><span className="text-gray-500">Alerts</span><span>{alerts.length}</span></div>
                        <div className="flex justify-between"><span className="text-gray-500">Funding</span><span>{marketStats.funding}</span></div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="border-t border-zinc-800 p-3">
                    <div className="flex gap-2">
                      <input
                        ...
                        className="w-full rounded-xl border border-zinc-800 bg-black px-4 py-3 text-sm outline-none focus:border-yellow-700"
                      />
                      <button onClick={() => sendAIMessage()} className="rounded-xl bg-yellow-600 px-5 py-3 text-sm font-black text-black hover:bg-yellow-500">Send</button>
                    </div>
                  </div>
                </div>

                <div className="border-l border-zinc-800 bg-black/35 p-4 space-y-3 overflow-auto">
                  ...
                  <div className="rounded-2xl border border-yellow-700/25 bg-yellow-500/5 p-4">
                    <p className="text-xs text-yellow-500 mb-2">AI Guardrail</p>
                    <p className="text-xs leading-5 text-gray-400">This phase uses the live dashboard context and local Wolvrene rules. External AI/API connection comes in the next phases.</p>
                  </div>
                </div>
              </div>
            </div>
