// Upgraded automatically with UI text cleanup, mood states, and naming polish
"use client";

// WOLVRENE v31 Prime Terminal Full Upgrade — FULL UPGRADE BUILD
...
type SignalState = "WAITING FOR SETUP" | "WATCHING LONG" | "WATCHING SHORT" | "ENTER LONG" | "ENTER SHORT" | "WAIT";
type RiskState = "LOW" | "CONTROLLED" | "ELEVATED" | "HIGH" | "WAIT";
...
type CandleSummary = {
  candles: number;
  trend: "BULLISH" | "BEARISH" | "MIXED" | "WAITING FOR SETUP";
  lastClose: number;
  high: number;
  low: number;
  rangePct: number;
...
type StructureBias = "BULLISH" | "BEARISH" | "RANGING" | "WAITING FOR SETUP";
type StructureEvent = "BOS_UP" | "BOS_DOWN" | "CHOCH_UP" | "CHOCH_DOWN" | "SWEEP_LOW" | "SWEEP_HIGH" | "RECLAIM" | "REJECTION" | "NONE";
type LiquidityBias = "BUY_SIDE_TAKEN" | "SELL_SIDE_TAKEN" | "BALANCED" | "WAITING FOR SETUP";
...
export default function WolvreneTerminal() {
  ...
  const candlesSummary = useMemo<CandleSummary>(() => {
    ...
    if (sample.length < 4) {
      return { candles: sample.length, trend: "WAITING FOR SETUP", lastClose: 0, high: 0, low: 0, rangePct: 0, avgBodyPct: 0, volatility: "NORMAL", impulse: "NONE" };
    }
  ...
  const structureState = useMemo<MarketStructureState>(() => {
    ...
    if (sample.length < 12) {
      return { bias: "WAITING FOR SETUP", lastSwingHigh: null, lastSwingLow: null, previousSwingHigh: null, previousSwingLow: null, event: "NONE", sweep: null, score: 0, summary: "Waiting for enough candles to build structure." };
    }
  ...
  const liquidityState = useMemo<LiquidityState>(() => {
    ...
    if (sample.length < 12) {
      return { bias: "WAITING FOR SETUP", sweptHigh: false, sweptLow: false, trapDirection: null, buySideLevel: null, sellSideLevel: null, score: 0, summary: "Waiting for liquidity context." };
    }
  ...
  const signalPlan = useMemo<SignalPlan>(() => {
    ...
    if (!mark || !direction) {
      return {
        state: mark ? "WAIT" : "WAITING FOR SETUP",
        direction: null,
    ...
    const state: SignalState =
      score >= 78
        ...
        : score >= 54
        ? direction === "LONG" ? "WATCHING LONG" : "WATCHING SHORT"
        : "WAIT";

    const risk: RiskState =
      state === "WAIT" ? "WAIT" : score >= 78 ? "LOW" : score >= 62 ? "CONTROLLED" : score >= 54 ? "ELEVATED" : "HIGH";
    ...
    return {
      ...
      markerPrice: entry,
      shouldMark: state === "WATCHING LONG" || state === "WATCHING SHORT" || state === "ENTER LONG" || state === "ENTER SHORT",
    };
  ...
  const rawDecisionPlan = useMemo<DecisionPlan>(() => {
    ...
    const phase: DecisionPhase = !decisionSettings.enabled
      ? signalPlan.state === "WAIT" || signalPlan.state === "WAITING FOR SETUP" ? "SCANNING" : "SPAWNED"
      : !mark || !direction
      ? "SCANNING"
  ...
  const visualIntelligence = useMemo(() => {
    ...
    if (sample.length < 12) {
      return {
        ...
        currentStructure: "WAITING FOR SETUP" as StructureBias,
  ...
  const v25FinalBrain = useMemo(() => {
    ...
    const activeTradeState =
      ...
      action === "WAIT RETEST" ? "ARMED WAITING FOR SETUP" :
  ...
  function useSignalPlan() {
    if (!signalPlan.direction || !signalPlan.entry || signalPlan.state === "WAIT" || signalPlan.state === "WAITING FOR SETUP") return;
  ...
  useEffect(() => {
    ...
    const stateForMarker: SignalState = direction === "LONG"
      ? plan.phase === "EXECUTE" ? "ENTER LONG" : "WATCHING LONG"
      : plan.phase === "EXECUTE" ? "ENTER SHORT" : "WATCHING SHORT";
...
// Finalized pack: signal labels polished, premium terminology, ready for user review.
