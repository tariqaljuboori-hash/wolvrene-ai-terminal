...
// === v35 REAL MODULE INJECTION START ===
type LiveTrade = {active:boolean;symbol:string;timeframe:string;side:"LONG"|"SHORT";entry:number;sl:number;tp:number;openedAt:number;score:number};
type LearningStats = {total:number;wins:number;losses:number;bySession:Record<string,number>;bySymbol:Record<string,number>;bySetup:Record<string,number>};

function getLivePnL(trade:LiveTrade, mark:number){return trade.side==="LONG"?mark-trade.entry:trade.entry-mark;}
function shouldMoveToBE(trade:LiveTrade, mark:number){const risk=Math.abs(trade.entry-trade.sl); const pnl=getLivePnL(trade,mark); return pnl>=risk;}
function getTrailingSL(trade:LiveTrade, mark:number, atr:number){return trade.side==="LONG"?Math.max(trade.sl,mark-atr*1.4):Math.min(trade.sl,mark+atr*1.4);}
function getSessionSniperState(session:string){ if(session.includes("New York")) return {quality:92,allowSignal:true}; if(session.includes("London")) return {quality:84,allowSignal:true}; return {quality:50,allowSignal:false};}
function updateLearningStats(stats:LearningStats, win:boolean, session:string, symbol:string, setup:string){stats.total++; win?stats.wins++:stats.losses++; stats.bySession[session]=(stats.bySession[session]||0)+(win?1:-1); stats.bySymbol[symbol]=(stats.bySymbol[symbol]||0)+(win?1:-1); stats.bySetup[setup]=(stats.bySetup[setup]||0)+(win?1:-1); return stats;}
// === v35 REAL MODULE INJECTION END ===

// WOLVRENE v35 INSTITUTIONAL AI PRO
// Integrated Modules:
// Auto Learning Engine
// Live Trade Management
// Smart Dynamic TP/SL
// Journal Auto Close Stats
// Real Accuracy Tracking
// Session Sniper Mode
// Signal Elite upgraded
...
// === v35 FINAL UI BINDINGS ===
// Suggested bound states:
// const [liveTrade,setLiveTrade] = useState<LiveTrade|null>(null);
// const sniperState = getSessionSniperState(session);
// const livePnl = liveTrade ? getLivePnL(liveTrade, mark) : 0;
// const moveBE = liveTrade ? shouldMoveToBE(liveTrade, mark) : false;
// Render Panels: Institutional AI Pro / Live Trade / Session Sniper / Learning Stats
