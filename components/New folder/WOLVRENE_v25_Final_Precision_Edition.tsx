...
// WOLVRENE v25 Final Precision Edition
...
export default function WolvreneTerminal() {
  ...
  const v25FinalBrain = useMemo(() => {
    ...
    return {
      ...
      stateColorClass: action === "CANCELLED" ? "text-gray-400" : finalDirection === "LONG" ? "text-green-400" : finalDirection === "SHORT" ? "text-red-400" : "text-yellow-400",
      ...
      summary: `v25 Master ${masterGrade} · ${action} · ${finalDirection || "WAIT"} · ${masterScore}% · votes L:${longVotes}/S:${shortVotes}`,
    };
  ...
  function buildCompactDiscordSignal(label = "WOLVRENE SIGNAL") {
    ...
    return [
      ...
      `Decision: ${v25FinalBrain.action} · v24: ${v25FinalBrain.masterGrade} ${v25FinalBrain.masterScore}% · Trigger: ${v25FinalBrain.triggerAligned ? "SYNCED" : "WAIT"}`,
  ...
  function useSignalPlan() {
    ...
    addJournal(`Signal used: ${v25FinalBrain.consoleState} at ${formatPrice(signalPlan.entry)} — risk ${signalPlan.risk}`);
  ...
  const aiInsights = useMemo(() => {
    ...
    notes.push(`v25 Master Brain: ${v25FinalBrain.summary}. Reason: ${v25FinalBrain.reason}`);
              ...
              >
                ...
                <div className="absolute left-3 top-3 z-40 max-w-[360px] rounded-2xl border border-yellow-500/20 bg-black/70 p-3 backdrop-blur-md shadow-[0_0_30px_rgba(0,0,0,0.65)] pointer-events-none">
                  <div className="flex items-center justify-between gap-3">
                    <p className="text-[10px] font-black tracking-[0.18em] text-yellow-400">v25 FINAL PRECISION</p>
                  ...
                  <p className="mt-2 text-[11px] text-gray-300">{v25FinalBrain.summary}</p>
                  <p className="mt-1 text-[10px] text-gray-500">{visualIntelligence.summary}</p>
                  <p className="mt-1 text-[10px] text-gray-400">{v25FinalBrain.reason}</p>\n                  <p className="mt-1 text-[10px] text-yellow-300">State: {v25FinalBrain.activeTradeState} · TP hits: {v25FinalBrain.tpHitCount}</p>
                  <p className="mt-1 text-[10px] text-gray-500">Final sync · precision score · active trade state · TP tracker · clean alerts</p>
                </div>
            ...
            <aside className="space-y-4 min-w-0">
              <div className={`${card} p-5`}>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-[10px] px-2 py-1 rounded-full border border-yellow-600/40 text-yellow-500 bg-yellow-500/10">V25</span>
                </div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
                  ...
                  <div className="rounded-xl border border-zinc-800 bg-black/60 p-3">
                    ...
                    <p className={v25FinalBrain.directionColorClass}>
                      {v25FinalBrain.finalDirection || "WAIT"}
                    </p>
                  </div>
                </div>
                ...
                {decisionSettings.showDecisionPanel && (
                  <div className="mt-4 rounded-2xl border border-yellow-700/30 bg-yellow-500/[0.045] p-3 text-xs space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-yellow-500 font-black">v25 DECISION BRAIN</span>
