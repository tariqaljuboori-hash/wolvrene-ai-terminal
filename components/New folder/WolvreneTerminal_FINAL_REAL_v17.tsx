...
const defaultExternalAlertSettings: ExternalAlertSettings = { discordWebhook: "", telegramWebhook: "", emailWebhook: "", enabled: false };
...
export default function WolvreneTerminal() {
  ...
  async function sendExternalAlert(title: string, body: string) {
    if (!externalAlertSettings.enabled || externalAlertBusyRef.current) return;
    const targets = [externalAlertSettings.discordWebhook, externalAlertSettings.telegramWebhook, externalAlertSettings.emailWebhook].filter(Boolean);
    if (!targets.length) return;
    externalAlertBusyRef.current = true;
    try {
      await Promise.allSettled(targets.map((url) => fetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ content: `🐺 **${title}**\n${body}`, title, body, symbol: "BTCUSDT", timeframe, session, price: livePrice || lastCandleRef.current?.close || 0 }) })));
    } catch (error) { console.warn("External alert failed", error); }
    finally { externalAlertBusyRef.current = false; }
  }
      ...
      <div className={fullscreen ? "w-full max-w-[1800px] mx-auto" : "w-full max-w-7xl mx-auto"}>
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-gray-500 text-xs tracking-wide">BTC AI Dashboard v16.7 — Fixed Signal Markers</p>
          </div>
              ...
              <div className={`${card} p-5`}>
                ...
                <div className="grid grid-cols-2 gap-2 mt-4 text-xs">
                  ...
                  <button
                    ...
                    className="col-span-2 rounded-xl border border-green-700/50 bg-green-500/10 p-3 text-green-400 hover:bg-green-500/20 disabled:cursor-not-allowed disabled:border-zinc-800 disabled:bg-black disabled:text-gray-600"
                  >
                    Use Signal
                  </button>
                </div>
