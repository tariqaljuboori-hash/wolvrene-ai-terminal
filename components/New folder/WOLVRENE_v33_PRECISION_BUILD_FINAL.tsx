...
// WOLVRENE v33 PRECISION BUILD — FINAL FULL SYSTEM
...
const TRADE_SYMBOLS: TradeSymbol[] = [
  { symbol: selectedSymbol, label: "BTCUSDT.P", icon: "₿" },
...
export default function WolvreneTerminal() {
  ...
  const reloadCandles = useCallback(async () => {
    try {
      const candles = await getBitgetCandles(timeframeRef.current, selectedSymbolRef.current);
  ...
  async function getMarketStats() {
    try {
      const row = await getBitgetTickerStats(selectedSymbolRef.current);
      setMarketStats({
