...
export async function POST(req: NextRequest) {
  try {
    ...
    const mergedLiveContext = {
      ...
      smartFib: liveContext?.smartFib || context?.smartFib || {},
    ...
    if (process.env.NODE_ENV !== "production") {
      console.log("AI_ROUTE_RECEIVED_CONTEXT", {
        ...
        smartFibMapState: ((mergedLiveContext as AIContext)?.smartFib as AIContext)?.mapState ?? null,
        smartFibDirection: ((mergedLiveContext as AIContext)?.smartFib as AIContext)?.mapDirection ?? null,
        radarConfidence,
