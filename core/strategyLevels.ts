...
export function classifyStrategy(input: StrategyInput): StrategyProfile {
  const confirmations = [
    ...
    { key: "clear invalidation", pass: input.invalidation !== null },
  ];
  ...
  const quality = Math.max(
    ...
    Math.min(
      100,
      Math.round(
        (input.structureScore + input.liquidityScore + input.triggerScore + input.sessionQuality) / 4
      )
    )
  );
  ...
  const base = {
    ...
    invalidationLogic:
      input.invalidation !== null
        ? `Invalidate thesis if price reaches ${input.invalidation}.`
        : "No clean invalidation level yet.",
